...
- 登録したスポットの半径150m以内に外から入ると、アプリを閉じていても
  通知＋バイブで知らせる。同一スポットの再通知クールダウンは既定3時間だが、
  30分〜12時間（30/60/180/360/720分）から選べ、通知可能な曜日・時間帯も
  設定できる（`NotificationScheduleDialog`）。
...
- 通知からの「寄った」記録、一覧表示、削除、空状態UI（Serendipity Log、
  `SPOT-04-S02-T01`・`T02`実装済み）。記録内容（spotId/spotTitle/recordedAt）を
  書き換える編集機能は無く、誤記録の訂正は「削除して、必要なら通知から改めて
  記録し直す」の1本（通知の「取り消す」と同じ削除操作を一覧画面からも行える）。
...
- 位置情報・通知・署名まわりの変更ごとに、実機確認Human Request
  （`CLAUDE.md`「実機確認が要る変更」節）が合格すること。
