...
・徒歩ルートを表示する操作をしたときだけ、その時点の現在地をGoogleのルート計算サービスへ送信します
...
| 位置情報を第三者と共有するか | **共有する（Google Directions API）**。徒歩ルートを表示する操作（通知タップ経由の`focusSpot`、地図上のスポットカード「経路」ボタン経由の`requestRoute`、いずれも`fetchRouteToSpot`に合流）をしたときだけ、その時点の現在地をルート計算のためGoogleへ送信する。広告・分析目的ではない（`SpotViewModel.kt`・`DirectionsRepository.kt`）。近接判定（ジオフェンス）自体は端末上で完結し送信しない |
