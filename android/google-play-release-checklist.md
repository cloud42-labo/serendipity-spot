...
・通知から徒歩ルートを表示するときだけ、その時点の現在地をGoogleのルート計算サービスへ送信します
...
| 位置情報を第三者と共有するか | **共有する（Google Directions API）**。通知から徒歩ルートを表示する操作をしたときだけ、その時点の現在地をルート計算のためGoogleへ送信する。広告・分析目的ではない（`DirectionsRepository.kt`）。近接判定（ジオフェンス）自体は端末上で完結し送信しない |
| 位置情報の用途 | アプリの中核機能（ジオフェンス通知、徒歩ルート表示） |
| 位置情報は端末外に送信されるか | **送信される**（上記の徒歩ルート表示時のみ、Google Directions APIへ）。ジオフェンスの近接判定自体は端末上でのみ行われ送信しない |
