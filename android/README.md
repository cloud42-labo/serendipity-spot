...
たまたま近くに来たときに「ついでに寄れる」と教える。目的地なら自分で行くので通知は要らない。
目的地ではないからこそ通知が要る、というのがこのアプリの成り立ち。

> 開発当初の名前は `Serendipity Spot`。パッケージ名（`com.cloud42labo.serendipityspot`）と
> スプレッドシート名（`Serendipity Spot`）は当時のまま残している。前者はOAuthクライアントと
> APIキーの制限に登録済みで、後者はDrive上の既存データを探す検索キーのため、
> どちらも変えると動かなくなる。表示名だけを変えている。

## この版でやっていること

- **Googleアカウントでサインイン**。認証は Credential Manager（Sign in with Google）、
  スプレッドシートを読み書きするためのスコープ取得は AuthorizationClient と、役割を分けている。
  要求するスコープは **`drive.file` のみ**（`spreadsheets` は要求しない。詳細は下記）
...
- 地図（Google Maps SDK）をタップしてスポットを登録。**右下の「＋」でも地図の中心に
  登録できる**（地図中央に照準を出している）。目印は**旗**で、
  登録済みは濃く、検索で出ただけの候補は薄い「まだ立っていない旗」として表示する。
  薄い旗をタップすると登録に進む
  - 「＋」は Maps SDK のタップ経路に依存しない入口で、地図タップが効かないときの
    切り分けにも使える。どのタップが拾われたかは診断欄の「最後の操作」に出る
- **バックグラウンドでの近接通知**（Geofencing API）。アプリを閉じていても、登録したスポットの
  半径150m以内に**外から入る**と通知＋バイブで知らせる。同じスポットは3時間再通知しない
- 入った通知を見逃しても、**2分後にまだ圏内にいれば「まだ近くです」ともう一度知らせる**
  （Geofencing の DWELL 遷移）。二度目は同じ滞在の続きのときだけで、1回に限る
- 住所・駅名・施設名での**検索**（上部バーの虫めがね）
- ピンの地点の**ストリートビュー**（Googleマップアプリを開く。アプリ内にパノラマを
  埋め込む実装も試したが、Maps Platform の課金対象になるためやめた）
- 登録済みスポットの一覧・**編集・削除**（下部シート）
- 端末再起動後もジオフェンスを張り直す（ローカルキャッシュから復元）

## 元のHTML版からの変更点・省略した機能

- 住所・スポット名でのテキスト検索は Android 標準の `Geocoder` で実装した（元のHTML版はNominatim）
...
「Google 認証プラットフォーム」（旧「OAuth同意画面」）で、アプリ名・ホームページ・
プライバシーポリシーURL・承認済みドメインを設定する。

**自分だけで使うなら**「テスト」状態のまま、自分のアカウントをテストユーザーに追加すれば動く。

**他の人にも配るなら「本番環境」に切り替える。** テスト状態には
**リフレッシュトークンが7日で失効する**制約があり、利用者が週一回ログインし直すことになる。
この制約は審査の有無ではなく**公開ステータス**に紐づくので、本番へ切り替えるだけで解ける。

> **要求するスコープを `drive.file` だけに保つこと。**
> `spreadsheets` は「機密性の高いスコープ」で、要求すると利用者に
> 「このアプリは Google で確認されていません」の警告画面が出て、消すにはGoogleの審査
> （2〜6週間）が要る。このアプリが使う Sheets API のメソッドは
> （create / get / values.get / values.append / values.update / batchUpdate のいずれも）
> `drive.file` を受け付けるため、`spreadsheets` は不要。
> 自分が作ったファイルしか触らない設計なら、権限としてもこちらの方が狭い。
...
> **終わったら必ず解除すること。**
> 「設定」→「開発者向けオプション」→「仮の現在地情報アプリを選択」→「なし」。
> アプリを消しただけでは足りない。解除しないと端末は偽装地点を返し続け、
> **実際に移動しても位置が動かない**。ジオフェンス・地図・現在地が同時に壊れ、
> 原因の切り分けが不可能になる。実際にこれで数時間溶かしている。
...
4. ついでにスポット を起動し、スポットが読み込まれた状態にする（ジオフェンスが張られる）
5. アプリを閉じる
6. 位置偽装アプリで、スポットの座標へ移動する
7. **確認が済んだら、上の注意のとおり偽装を解除する**

ジオフェンスは「外から中への遷移」で発火するので、**最初に外にいることが必要**。
スポットの真上から始めても、圏内でアプリを開き直しても鳴らない
（登録時の即時発火は `v0.4.1` で無効にしている）。
...
- 端末メーカーによっては省電力設定でバックグラウンドの位置情報監視が止められることがある
  ...
  実機で通知が来ない場合はまずここを疑う）
- ジオフェンスは1アプリ最大100件（Android/Google Playの制限）。**スポットは101件目以降も
  登録・一覧表示できるが、通知が届くのは新しい100件まで**で、古い方から順に通知の対象外になる。
  現在の件数と上限はアプリの下部シート（「登録スポット（N / 100）」）に出る
  - `v0.11.4` より前は、実装が `take(100)` で**古い方**100件を残しており、新しく登録した
    スポットほど通知されないという逆の挙動になっていた
